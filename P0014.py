...
class avltree(object):    
    ...
    def search(self, value):
        if self.node is None:
            return False
        nodeValueTuple = self.node.key
        nodeValue = nodeValueTuple[0]
        # print(nodeValue)
        if value == nodeValue:
            return nodeValueTuple
...
# tree = avltree()
def find_collatz(value):
    collatzResult = tree.search(value)
    if collatzResult:
        return collatzResult
    if value % 2 == 0:
        collatzResult = find_collatz(value // 2)
    else:
        collatzResult = find_collatz(value * 3 + 1)
    collatzResult = (value, collatzResult[1] + 1)
    # print(collatzResult)
    # print((value, collatzResult[1]+1))
    # tree.insert(value)
    tree.insert(collatzResult)
    return (collatzResult)
...
if __name__ == "__main__":
    ...
    # # shuffle(data)
    # for key in data:
        # tupleValue = (key, randrange(1,10))
        # tupleValue = (key, key)
        # print(tupleValue)
        # tree.insert(tupleValue)
    ...
    tree.display()
    maxResult = (1, 1)
    testrange = range(2, 999999)
    for i in testrange:
        current = find_collatz(i)
        if current[1] > maxResult[1]:
            maxResult = current
    print(maxResult)
